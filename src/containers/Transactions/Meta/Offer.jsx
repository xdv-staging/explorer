    ...
    meta.push(
      <li key={`taker_pays_decreased_${index}`} className="meta-line">
        ...
        <Trans Trans i18nKey="decreased_from_to">
          ...
          to
          <b>{{ final: localizeNumber(normalize(finalPays, paysCurrency), language, options) }}</b>
        </Trans>
      </li>
    );
  }
    ...
    meta.push(
      <li key={`taker_gets_decreased_${index}`} className="meta-line">
        ...
        <Trans Trans i18nKey="decreased_from_to">
          ...
          to
          <b>{{ final: localizeNumber(normalize(finalGets, getsCurrency), language, options) }}</b>
        </Trans>
      </li>
    );
  }

  return <React.Fragment key={`renderOfferChangesMeta_${index}`}>{meta}</React.Fragment>;
};
