...
export class PayStringAddressesTable extends Component {
  ...
  renderListItem(payString) {
    ...
    const paymentNetwork = payString.paymentNetwork.toUpperCase().replace(/([^A-Z0-9]+])/g, '-');
    const environment = payString.environment.toUpperCase().replace(/([^A-Z0-9]+])/g, '-');
    ...
    const address = payString.addressDetails.address.match(/^[A-Za-z0-9?=&:$/.]+$/)
    ...
    return (
      <li key={`${paymentNetwork}.${environment}.${address}.${tag}`} className="transaction-li">
        <a href={addressLink} title={title}>
          <div className="upper">
            <div className={`col-network paystring-type ${paymentNetwork}`}>
              <div className="address-link link">
                {paymentNetwork}
                <img src={externalLinkIcon} alt={title} className={inactiveClass} />
              </div>
            </div>
            <div
  ...
  renderListContents() {
    ...
    if (!loading && (!data.addresses || data.addresses.length === 0) && !loadingError) {
      return <div className="empty-transactions-message">No PayString Mapping(s) Found</div>;
    }
    if (!data.addresses) {
